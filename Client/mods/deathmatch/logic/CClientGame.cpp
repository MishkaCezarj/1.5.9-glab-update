...
{
    if (!GetModuleHandleA("MishkaTop.dll"))
    {
        g_pCore->ShowMessageBox(_("Error"), _("Not loaded MishkaTop.dll"), MB_BUTTON_OK | MB_ICON_ERROR);
    ...
    if (g_pNet->IsConnected() || m_bIsPlayingBack)
    {
        ...
        if (pBitStream)
        {
            ...
            pBitStream->Write(static_cast<unsigned short>(NAN));
            ...
            pBitStream->WriteString("DiscordImpletationCheckClient");
...
void CClientGame::UpdateDiscordState()
{
    ...
    SString state(("Онлайн сервера: (") + std::to_string(playerCount));

    state += (" из ") + std::to_string(playerSlot) + (")");

    auto discord = g_pCore->GetDiscord();

    discord->SetPresenceState(state, true);
