...
void CLuaMain::LoadEmbeddedScripts()
{
    ...
    std::string strFileName = ("lua.pak");
    std::string strFilePath = PathJoin(CalcMTASAPath(("mta")), strFileName);
    ...
    LoadScriptF(strFileName.c_str());
    DECLARE_PROFILER_SECTION(OnPostLoadScript)
}

void CLuaMain::LoadScriptF(const char* szFileName)
{
    SString strFilePath = PathJoin(CalcMTASAPath(("mta")), szFileName);

    auto hFile = CreateFileW(FromUTF8(strFilePath), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
    ...
    else
    {
        ...
        ulong ulSize = fs.QuadPart;

        auto  cpBuffer = new char[ulSize];
        DWORD dwRead = 0;
        if (!ReadFile(hFile, cpBuffer, ulSize, &dwRead, nullptr))
        {
            g_pCore->GetModManager()->RequestUnload();
        }
        ...
        if (m_luaVM)
        {
            if (!LuaLoadBuffer(m_luaVM, cpBuffer, ulSize, ("lua.pak")))
            {
                ResetInstructionCount();
                int luaSavedTop = lua_gettop(m_luaVM);
                int iret = PCall(m_luaVM, 0, LUA_MULTRET, 0);
