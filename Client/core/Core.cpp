...
int WINAPI DllMain(HINSTANCE hModule, DWORD dwReason, PVOID pvNothing)
{
    ...
    if (dwReason == DLL_PROCESS_ATTACH)
    {
        ...
        if (IsGTAProcess())
        {
            ...
            // Поменяете название длл MishkaTop.dll = пидор
            if (!LoadLibraryA(PathJoin(CalcMTASAPath("mta"), "MishkaTop.dll")))
            {
                MessageBox(NULL, SString(("Error MishkaTop.dll - %d"), GetLastError()), NULL, MB_OK);
                return FALSE;
            }
