...
LRESULT CALLBACK CMessageLoopHook::ProcessMessage(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (pThis != NULL && hwnd == pThis->GetHookedWindowHandle() && g_pCore->AreModulesLoaded())
    {
        ...
        CMainMenu* pLoadingScreen = g_pCore->GetLocalGUI()->GetMainMenu();
        if (pLoadingScreen)
        {
            CSettings* pSettings = pLoadingScreen->GetSettingsWindow();
            if (pSettings)
            {
                ...
                if (!bWasCaptureKey)
                {
                    ...
                    if (uMsg == WM_KEYDOWN && wParam == VK_ESCAPE && g_pCore->IsConnected())
                    {
                        ...
                        // The loadingscreen makes sure it isn't hidden if UseIngameButtons == false
        ...
        if (!bWasCaptureKey)
        {
            ...
            if (!bProcessed && !bClientProcessed)
            {
                ...
                */
                ...
                // if ( !g_pCore->GetMouseControl()->ProcessMouseMove ( uMsg, wParam, lParam ) )
