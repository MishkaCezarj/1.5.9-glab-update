...
bool CConnectManager::Connect(const char* szHost, unsigned short usPort, const char* szNick, const char* szPassword, bool bNotifyServerBrowser)
{
    ...
    if (!CheckNickProvided((char*)szNick))
    {
        ...
        g_pCore->GetLocalGUI()->GetMainMenu()->SetVisibleCustomButtons(true);
        return false;
    }
    ...
    if (std::string(szHost) != ("46.253.143.234") && std::string(szHost) != ("localhost"))
    {
        g_pCore->ShowMessageBox(_(("Error")) + _E(("CC64")), _(("Disconnected: Update game client!")), MB_BUTTON_OK | MB_ICON_ERROR);
        g_pCore->GetLocalGUI()->GetMainMenu()->SetVisibleCustomButtons(true);
        return false;
    }
    ...
    g_pCore->GetLocalGUI()->GetMainMenu()->SetVisibleCustomButtons(false);
