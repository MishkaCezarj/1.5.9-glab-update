...
CCore::CCore()
{
    ...
    m_DiscordManager = std::shared_ptr<CDiscordManager>(new CDiscordManager());
}
...
void CCore::SetConnected(bool bConnected)
{
    ...
    if (bConnected)
    {
        ...
        m_pLocalGUI->GetMainMenu()->m_ServerBrowser.SetVisible(false);
    }
    else
    {
        GetLocalGUI()->GetMainMenu()->SetVisibleCustomButtons(true);
    }
}
...
{
    CMainMenu* pLoadingScreen = GetLocalGUI()->GetMainMenu();
    CVector2D  ScreenSize = m_pGUI->GetResolution();
    RemoveMessageBox();

    if (szTitle == nullptr)
    {
        if (szText == _(("Download map...")) || szText == _(("Download resources...")))
        {
            m_pMessageBox = m_pGUI->CreateLabel(pLoadingScreen->m_pBackground, szText);
            m_pMessageBox->SetSize(CVector2D(ScreenSize.fX / 1.8, 37), false);
            m_pMessageBox->SetPosition(CVector2D(40, ScreenSize.fY - 98), false);
            m_pMessageBox->SetFont(("open-sans"));
            m_pMessageBox->SetHorizontalAlign(CGUI_ALIGN_LEFT);
            m_pMessageBox->SetVerticalAlign(CGUI_ALIGN_VERTICALCENTER);
        }
        else
        {
            m_pMessageBox = m_pGUI->CreateLabel(pLoadingScreen->m_pBackground, szText);
            m_pMessageBox->SetSize(CVector2D(ScreenSize.fX / 1.8, 37), false);
            m_pMessageBox->SetPosition(CVector2D(40, ScreenSize.fY - 73), false);
            m_pMessageBox->SetFont(("open-sans"));
            m_pMessageBox->SetHorizontalAlign(CGUI_ALIGN_LEFT);
            m_pMessageBox->SetVerticalAlign(CGUI_ALIGN_VERTICALCENTER);
        }
    ...
    {
        m_pMessageBox = m_pGUI->CreateLabel(pLoadingScreen->m_pBackground, SString(("%s: %s"), szTitle, szText));
        m_pMessageBox->SetSize(CVector2D(ScreenSize.fX, 60), false);
        m_pMessageBox->SetPosition(CVector2D(40, ScreenSize.fY - 73), false);
        m_pMessageBox->SetFont(("open-sans"));
        m_pMessageBox->SetHorizontalAlign(CGUI_ALIGN_LEFT);
        m_pMessageBox->SetVerticalAlign(CGUI_ALIGN_VERTICALCENTER);
...
void CCore::ShowErrorMessageBox(const SString& strTitle, SString strMessage, const SString& strTroubleLink)
{
    ...
    {
        g_pCore->ShowMessageBox(strTitle, strMessage, MB_BUTTON_OK | MB_ICON_ERROR);
    }
    else
    {
...
{
    uint uiErrorCode = g_pCore->GetNetwork()->GetExtendedErrorCode();
    if (uiErrorCode != 0)
    {
...
void CCore::CreateNetwork()
{
    ...
    string nNick = GenerateHashHexString(EHashFunctionType::MD5, szSerial);
    CVARS_SET(("nick"), std::string(nNick).substr(0, 22));
    SString strLocale = ("ru");
...
void CCore::DoPostFramePulse()
{
    ...
    if (const long long ticks = GetTickCount64_(); ticks > m_timeDiscordAppLastUpdate + TIME_DISCORD_UPDATE_RICH_PRESENCE_RATE)
    {
        if (const auto discord = g_pCore->GetDiscord(); discord && discord->IsDiscordRPCEnabled())
        {
            ...
            discord->SetPresenceButtons(1, ("Discord server"), ("https://discord.gg/sMHHRJUGQF"));
...
void CCore::UpdateRecentlyPlayed()
{
    ...
    {
        CServerBrowser* pServerBrowser = g_pCore->GetLocalGUI()->GetMainMenu()->GetServerBrowser();
...
void CCore::RecalculateFrameRateLimit(uint uiServerFrameRateLimit, bool bLogToConsole)
{
    ...
    if (bLogToConsole)
    {
        ...
        g_pCore->GetConsole()->Print(strStatus);
    }
}
...
void CCore::UpdateDummyProgress(int iValue, const char* szType)
{
    ...
    if (IsConnected())
    {
        if (m_iDummyProgressValue)
        {
            ...
            ShowMessageBox(nullptr, _(("sdfsdjfhksjdfhksdjhfksdhf")), MB_BUTTON_CANCEL | MB_ICON_INFO);
        }
        if (m_iDummyProgressValue == 100)
        {
            RemoveMessageBox();
        }
    }
...
{
    if (g_pCore->IsFocused() && !CLocalGUI::GetSingleton().IsMainMenuVisible())
...
void CCore::OpenSettings(bool bVisible)
{
    GetLocalGUI()->GetMainMenu()->m_Settings.SetVisible(bVisible);
}

std::shared_ptr<CDiscordManagerInterface> CCore::GetDiscord()
{
    return m_DiscordManager;
}

void CCore::SetLoadingScreen(bool bVisible)
{
    GetLocalGUI()->SetMainMenuVisible(bVisible);
}
