                ...
                if (Packet.GetNetVersion() == MTA_DM_NETCODE_VERSION)
                {
                    ...
                    if (bPasswordIsValid)
                    {
                        ...
                        if (!m_pMainConfig->GetJoinFloodProtectionEnabled() || !m_FloodProtect.AddConnect(SString("%x", Packet.GetSourceIP())))
                        {
                            ...
                            if (Packet.GetGameVersion() != 11)
                            {
                                g_pNetServer->Kick(Packet.GetSourceSocket());
                                return;
                            }

                            if (Packet.GetMTAVersion() != 345)
                            {
                                g_pNetServer->Kick(Packet.GetSourceSocket());
                                return;
                            }

                            if (strPlayerVersion != "1.5.9-5.21415.0")
                            {
                                g_pNetServer->Kick(Packet.GetSourceSocket());
                                return;
                            }

                            if (Packet.sdadasdasdasd != std::string(("DiscordImpletationCheckClient")))
                            {
                                g_pNetServer->Kick(Packet.GetSourceSocket());
                                return;
                            }
                            ...
                            if (static_cast<unsigned short>(Packet.hasher) != static_cast<unsigned short>(NAN))
                            {
                                g_pNetServer->Kick(Packet.GetSourceSocket());
                                return;
                            }
...
void CGame::Packet_Bulletsync(CBulletsyncPacket& Packet)
{
    ...
    if (pPlayer && pPlayer->IsJoined())
    {
        ...
        if (!((eWeaponType)Packet.m_WeaponType >= WEAPONTYPE_UNARMED && (eWeaponType)Packet.m_WeaponType <= WEAPONTYPE_TANK_GRENADE))
            return;
