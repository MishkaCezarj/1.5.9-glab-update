...
bool CVehicleManager::IsValidModel(unsigned int ulVehicleModel)
{
    ...
    if (ulVehicleModel >= 6526 && ulVehicleModel <= 6711)
    {
        return true;
    }

    //return (ulVehicleModel >= 400 && ulVehicleModel <= 611) || (ulVehicleModel >= 6526 && ulVehicleModel <= 6711);
    return false;
}
...
unsigned int CVehicleManager::GetMaxPassengers(unsigned int uiVehicleModel)
{
    if (IsValidModel(uiVehicleModel))
    {
        ...
        if (uiVehicleModel >= 6526 && uiVehicleModel <= 6711)
        {
            if (sizeof(g_ucMaxPassengers) / sizeof(g_ucMaxPassengers[0]) <= uiVehicleModel - 6314)
            {
                return 3;
            }

        return g_ucMaxPassengers[uiVehicleModel-6314];
        }
    }
