...
(
  ...
  echo sLinkFile = oWS.ExpandEnvironmentStrings^("BY MISHKA.sln.lnk"^)
  echo Set oLink = oWS.CreateShortcut^(sLinkFile^)
  echo oLink.TargetPath = oWS.ExpandEnvironmentStrings^("%~dp0\Build\BY MISHKA.sln"^)
  echo oLink.Save
) 1>%SCRIPTFILE%
